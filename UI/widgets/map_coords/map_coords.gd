...
func _process(delta: float) -> void:
	...
	if base_tile_x >= 0 and base_tile_x < Worldgen.heightmap.size():
		if base_tile_y >= 0 and base_tile_y < Worldgen.heightmap[base_tile_x].size():
			height_value = Worldgen.heightmap[base_tile_x][base_tile_y]
			biome_value = Worldgen.biomemap[base_tile_x][base_tile_y]
