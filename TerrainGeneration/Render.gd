...
var renderGrid = []
var renderGridSizeX: float = 65
var renderGridSizeY: float = 50
	...
	if Worldgen.world.size() > 0:
		for x in range(renderGridSizeX):
			for y in range(renderGridSizeY):
				...
				add_child(new_tile)
				if Worldgen.world[ \
						(renderGrid[x][y][0] + playerPosRoundedX)/tile_size_x] \
						[(renderGrid[x][y][1] + playerPosRoundedY)/tile_size_y \
					] < 0.5:
					# дно
					new_tile.get_child(0).get_child(0).texture = load("res://assets/tiles/water_bottom.png")
					new_tile.global_position = Vector2( \
						renderGrid[x][y][0] + playerPosRoundedX, \
						renderGrid[x][y][1] + playerPosRoundedY + 64 \
					)
					# вода
					...
					new_tile_water.global_position = Vector2( \
						renderGrid[x][y][0]+playerPosRoundedX, \
						renderGrid[x][y][1]+playerPosRoundedY + 24 \
					)
				else:
					# пол
					...
					new_tile.global_position = Vector2( \
						renderGrid[x][y][0]+playerPosRoundedX, \
						renderGrid[x][y][1]+playerPosRoundedY \
					)
